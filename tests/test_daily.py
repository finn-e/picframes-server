...
def test_daily_config_images_are_entry_titles(logged_in):
    ...
    logged_in.post(f'/admin/playlists/{pid}/rename_entry/{eids[0]}', json={'title': 'Beach Day'})
    r = logged_in.get('/api/frame-config', headers=device_headers())
    body = r.get_json()
...
def test_daily_config_respects_entry_orientation_toggle(logged_in):
    ...
    logged_in.post(f'/admin/entry-toggle-orient/{eids[0]}', json={'orient': 'l', 'enabled': False})
    r = logged_in.get('/api/frame-config', headers=device_headers())
...
def test_daily_zip_304_on_matching_version(logged_in):
    ...
    version = logged_in.get('/api/frame-config',
                            headers=device_headers()).get_json()['daily_zip_version']
    r = logged_in.get(f'/api/image-zip?version={version}', headers=device_headers())
    assert r.status_code == 304
    # Non-matching version → full zip
    r = logged_in.get('/api/image-zip?version=00000000', headers=device_headers())
    assert r.status_code == 200


def test_zip_version_changes_when_entries_change(logged_in):
    pid, eids, _ = setup_playlist_device(logged_in, bases=('alpha',))
    v1 = logged_in.get('/api/frame-config',
                       headers=device_headers()).get_json()['daily_zip_version']
    from tests.conftest import make_original
    make_original('gamma')
    r = logged_in.post(f'/admin/playlists/{pid}/add_image', json={'base': 'gamma'})
    assert r.status_code == 200
    v2 = logged_in.get('/api/frame-config',
                       headers=device_headers()).get_json()['daily_zip_version']
    assert v1 != v2
    # Old version now stale → zip served, not 304
    r = logged_in.get(f'/api/image-zip?version={v1}', headers=device_headers())
    assert r.status_code == 200
...
def test_zip_version_changes_on_rename(logged_in):
    ...
    v1 = logged_in.get('/api/frame-config',
                       headers=device_headers()).get_json()['daily_zip_version']
    logged_in.post(f'/admin/playlists/{pid}/rename_entry/{eids[0]}', json={'title': 'renamed'})
    v2 = logged_in.get('/api/frame-config',
                       headers=device_headers()).get_json()['daily_zip_version']
    assert v1 != v2
...
def test_fw_version_stored_from_header(logged_in):
    ...
    logged_in.get('/api/frame-config', headers=hdrs)
...
def test_fw_version_not_overwritten_if_same(logged_in):
    ...
    r = logged_in.get('/api/frame-config', headers=hdrs)
    assert r.status_code == 200
    cfg = db.load_config(owner_id=1)
    dev = next(d for d in cfg['devices'] if d['mac'] == TEST_MAC)
...
def test_daily_config_includes_debug(logged_in):
    ...
    logged_in.post('/admin/device_debug',

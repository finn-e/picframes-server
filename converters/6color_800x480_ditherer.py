#!/usr/bin/env python3
# ==========================================================================================
# DESCRIPTION: Floyd-Steinberg ditherer for landscape images (800x480). Dithers to 6-color Spectra 6 palette and packs as 4bpp RAW binary.
...
def process_image(src_path, dst_path):
    ...
    # Target ratio is 5:3 (1.6667)
    target_w, target_h = 800, 480
    target_ratio = target_w / target_h
    
    current_ratio = w / h
    if current_ratio > target_ratio:
        # Image is wider than 5:3 -> crop sides
        crop_h = h
        crop_w = int(h * target_ratio)
        x_offset = (w - crop_w) // 2
        y_offset = 0
    else:
        # Image is taller than 5:3 -> crop top/bottom
        crop_w = w
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 6color_800x480_ditherer.py <input_image> [output_bin]")
        sys.exit(1)
